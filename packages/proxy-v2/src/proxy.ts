...
export interface Proxy extends HandlerRecord {}
export interface Inject extends InjectableRecord {}
...
export function inject<T extends string>(prefix: T): Inject[T];
...
export function proxy<T extends keyof ToProxy<Proxy, never>>(
  prefix: T
): ToProxy<Proxy, never>[T];
export function proxy<T extends HandlerNode = Proxy>(): ToProxy<T, never>;
